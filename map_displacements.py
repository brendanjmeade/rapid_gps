...
axs[0].imshow(grid_e.T, cmap='RdYlGn', extent=(min_lon, max_lon, min_lat, max_lat), origin='lower')
axs[0].plot(points_X, points_Y, "k.", ms=1)
axs[0].set_title('East Displacement', weight='bold')
axs[0].set_ylabel('Latitude', weight='bold')
axs[1].imshow(grid_n.T, cmap='RdYlGn', extent=(min_lon, max_lon, min_lat, max_lat), origin='lower')
axs[1].plot(points_X, points_Y, "k.", ms=1)
axs[1].set_title('North Displacement', weight='bold')
axs[1].set_xlabel('Longitude', weight='bold')
axs[2].imshow(grid_v.T, cmap='RdYlGn', extent=(min_lon, max_lon, min_lat, max_lat), origin='lower')
axs[2].plot(points_X, points_Y, "k.", ms=1)
axs[2].set_title('Up Displacement', weight='bold')
fig.colorbar(axs[0].imshow(grid_e.T, cmap='RdYlGn', extent=(min_lon, max_lon, min_lat, max_lat), origin='lower'), ax=axs, orientation='horizontal')
plt.show()
